...
def _opt(argv: list, args: dict, flag: str, key: str, cast=str) -> None:
    """Append `flag <cast(args[key])>` when the field was filled in."""
    v = args.get(key)
    if v is not None and v != "":
        argv += [flag, str(cast(v))]


def _optflag(argv: list, args: dict, flag: str, key: str,
...
register(JobType(
    ...
    description="Turn a waypoint Course YAML into a canonical Trajectory NPZ "
                ...
                "policy. 'mpc' (default) rolls FiGS' VehicleRateMPC over a "
                "min-time-snap reference — dynamically feasible with real "
                "pitch/bank, which the renderer turns into realistic camera "
                "tilt; use it for anything feeding training data. 'spline' "
                "is a kinematic cubic spline (level camera, <1 s) — fine "
                "for quick geometry checks only. Every plan is validated "
                "against the scene's safety config (bounds/speed/tilt + "
                "drone-OBB collision); a violating plan is NOT saved. NPZ "
                "lands in runs/courses/.",
    fields=[
        ...
        _f("planner", "Planner", "select",
           options=["mpc", "spline"], default="mpc"),
...
def _build_eval_campaign(args: dict) -> Built:
    ...
    _optflag(argv, args, "--no-rtc", "no_rtc", default=True)
    rec = args.get("recovery_mode") or "default"
    if rec == "no-recovery":
        argv += ["--no-recovery"]
    elif rec == "force-recovery":
        argv += ["--force-recovery"]
    _optflag(argv, args, "--skip-flythrough", "skip_flythrough")
    _optflag(argv, args, "--resume", "resume")
    _opt(argv, args, "--execute-chunk-size", "execute_chunk_size", int)
    _opt(argv, args, "--out", "out")
...
def _build_recovery_collect(args: dict) -> Built:
    ...
    _opt(argv, args, "--perturbation-recipe", "perturbation_recipe")
    _opt(argv, args, "--collection-seed", "collection_seed", int)
    _optflag(argv, args, "--no-rtc", "no_rtc", default=True)
    _opt(argv, args, "--execute-chunk-size", "execute_chunk_size", int)
    _opt(argv, args, "--out", "out")
...
def _build_vla_episode(args: dict) -> Built:
    ...
    argv = ["-m", "falsify.cli.run_vla_episode",
            ...
            "--policy-config", args["policy_config"],
            "--out", out]
    _opt(argv, args, "--horizon-s", "horizon_s", float)
    _opt(argv, args, "--hz", "hz", int)
    _opt(argv, args, "--seed", "seed", int)
    _opt(argv, args, "--safety", "safety")
    _opt(argv, args, "--recovery", "recovery")
    _opt(argv, args, "--perturbations", "perturbations")
    label = f"{Path(args['policy_config']).stem} @ {Path(args['scene']).stem}"
...
def _build_export_training_data(args: dict) -> Built:
    ...
    argv = ["-m", "falsify.cli.export_training_data",
            ...
            "--out", args["out"]]
    _opt(argv, args, "--episode-index", "episode_index", int)
    _opt(argv, args, "--index-offset", "index_offset", int)
    _opt(argv, args, "--task-index", "task_index", int)
    _opt(argv, args, "--chunk-steps", "chunk_steps", int)
    _opt(argv, args, "--hz", "hz", float)
    if args.get("prompt_name"):
        argv += ["--prompt", configs_enum.prompt_task(args["prompt_name"])]
...
def _build_render_recoveries(args: dict) -> Built:
    ...
    argv = ["scripts/recovery/render_recoveries_to_dataset.py",
            ...
            "--embodiment", args["embodiment"],
            "--out", args["out"]]
    _opt(argv, args, "--episode-index-base", "episode_index_base", int)
    _opt(argv, args, "--index-offset", "index_offset", int)
    _opt(argv, args, "--task-index", "task_index", int)
    return Built(argv, out_dir=args["out"],
...
def _build_combine_lerobot(args: dict) -> Built:
    ...
    for spec in str(args["tasks"]).split(";"):
        ...
        if spec:
            argv += ["--task", spec]
    _opt(argv, args, "--drop-last-pattern", "drop_last_pattern")
    _opt(argv, args, "--fps", "fps", int)
    _optflag(argv, args, "--overwrite", "overwrite")
    return Built(argv, out_dir=args["out"],
...
def _build_campaign_grid(args: dict) -> Built:
    _require(args, "campaigns", "out")
    specs = args["campaigns"] if isinstance(args["campaigns"], list) \
        else [s.strip() for s in str(args["campaigns"]).split(";") if s.strip()]
    argv = ["scripts/eval/plot_campaign_grid.py", "--out", args["out"]]
    for s in specs:
        argv += ["--campaign", s]
    _opt(argv, args, "--title", "title")
    return Built(argv, out_dir=args["out"],
                 label=f"grid ({len(specs)} campaigns)")


register(JobType(
    name="campaign_grid",
    label="Campaign comparison grid (HTML)",
    kind="job", gpu=False,
    description="Faceted Plotly grid comparing several campaigns side by "
                "side (per-scene stacked outcome bars per cell, unified "
                "legend). Used automatically as the final step of an eval "
                "sweep; also launchable standalone on historical campaigns.",
    fields=[
        _f("campaigns", "Campaigns (';'-separated label:path)", "text",
           required=True,
           help="e.g. nh_right:runs/eval_campaigns/…/run-001-pure-…; …"),
        _f("out", "Output HTML path", "text", required=True),
        _f("title", "Title", "text"),
    ],
    build=_build_campaign_grid,
))
