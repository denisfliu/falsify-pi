...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    ap.add_argument("--no-drone-obb", dest="show_drone_obb",
                    ...
                    help="Skip emitting per-trial drone-body OBB traces. "
                         "By default OBB traces are emitted (hidden) with "
                         "a 'Drone OBB' toggle button in viz/trajectories.html.")
    ap.set_defaults(show_drone_obb=True)
    ap.add_argument("--obb-stride", type=int, default=25,
                    help="Draw one OBB every N rollout steps. Default 25 "
                         "(~1 box/s at 30 Hz).")
    ...
    if not bundle_dir.is_dir():
        raise SystemExit(f"bundle dir not found: {bundle_dir} "
                         f"(run scripts/eval/generate_eval_bundles.py first)")
    ...
    if not args.no_viz:
        try:
            ...
            print(f"[warn] viz emit failed: {e}  "
                  "(re-run with scripts/eval/plot_eval_run.py to retry)")
